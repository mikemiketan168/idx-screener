...
def _score_base(df, mode="general"):
    ...
    try:
        ...
        if mode in ["bpjs", "general", "bsjp"]:
            ...
            # swing/value: boleh lebih tenang
            if 1.5 <= atr_pct <= 5:
                score += 10
        ...
        # penalty / bonus kecil untuk mode tertentu
        if mode == "bpjs":
        ...
        with st.spinner(f"Analyzing {selected}..."):
            ...
            if df is None:
                ...
                if result is None:
                    ...
                    ⏰ Disesuaikan dengan strategi Chef (day trade / swing / value).
                    """
                    )
...
# --- SCREENER MODES ---
    ...
    if st.button("🚀 START SCAN", type="primary"):
        ...
        if df1.empty:
            st.error("❌ Tidak ada kandidat yang lolos filter.")
        else:
            st.markdown("### 🧪 Stage 1 – Top Candidates (by Score)")
            st.dataframe(
                df1[["Ticker", "Price", "Score", "Grade", "Trend", "Signal"]],
                use_container_width=True,
            )

            st.markdown("### 🏆 Stage 2 – Elite Picks (A+/A/B+)")
            st.dataframe(
                df2[["Ticker", "Price", "Score", "Grade", "Trend", "Signal", "Entry", "TP1", "TP2", "SL"]],
                use_container_width=True,
            )

            create_csv_download(df2, strategy)

            # Optional: pilih satu untuk lihat chart
            if not df2.empty:
                st.markdown("### 📈 Lihat Chart salah satu Elite Pick")
                pick = st.selectbox("Pilih Ticker", df2["Ticker"].tolist())
                if pick:
                    t_full = f"{pick}.JK"
                    df = fetch_data(t_full, period)
                    if df is not None:
                        chart = create_chart(df, pick)
                        if chart:
                            st.plotly_chart(chart, use_container_width=True)
